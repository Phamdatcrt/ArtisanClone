...
def content() -> str:
    ...
    tbl_Annotations.add_row(['~dDTs',QApplication.translate('HelpDlg','Number of seconds before or after DTs \nBest used inside double quotes (see notes below) \nNegative value before DTs \nDisplays &#39;*&#39; when no DTs event exists'),47])
    tbl_Annotations.add_row(['~dDTs_ms',QApplication.translate('HelpDlg','Time after DTs in min:sec \nBest used inside double quotes (see notes below) \nNegative value before DTs \nDisplays &#39;*&#39; when no DTs event exists'),'1:35'])
    tbl_Annotations.add_row(['~preDTs',QApplication.translate('HelpDlg','Number of seconds before DTs \nBest used inside single quotes or back ticks (see notes below) \nPositive value only \nDisplays &#39;*&#39; after DTs or when no DTs event exists'),50])
    tbl_Annotations.add_row(['~preDTs_ms',QApplication.translate('HelpDlg','Time before DTs  in min:sec \nBest used inside single quotes or back ticks (see notes below) \nPositive value only \nDisplays &#39;*&#39; after DTs or when no DTs event exists'),'1:25'])
    tbl_Annotations.add_row(['~DTR',QApplication.translate('HelpDlg','Development time ratio (percent). Note: DTR=0 before DTs \n100*(t{Event}-t{DTs})/(t{DTs}-t{CHARGE})'),12])
    ...
    tbl_Examples.add_row([QApplication.translate('HelpDlg','Gas ~E% &#39;@~Y2 ~degmode&#39;"@~DTR% DTR"'),QApplication.translate('HelpDlg','Before DTs:\nGas 50% @340 \u00b0F\n\nAfter DTs:\nGas 50% @12% DTR')])
    tbl_Examples.add_row([QApplication.translate('HelpDlg','Gas ~E% &#39;@~Y2 ~degmode`, ~preDTs sec before DTs`&#39;"@~DTR% DTR"'),QApplication.translate('HelpDlg','More than 90 seconds before DTs:\nGas 50% @340 \u00b0F\n\nLess than 90 seconds before DTs:\nGas 50% @340 \u00b0F, 50 sec before DTs \n\nAfter DTs:\nGas 50% @12% DTR')])
    ...
    tbl_Notes.add_row([QApplication.translate('HelpDlg','Anything between double quotes " will show only after DTs. Example: "~E1 @~DTR%"')])
    tbl_Notes.add_row([QApplication.translate('HelpDlg','Anything between single quotes &#39; will show only before DTs. Example: &#39;~E1 @~degmode&#39;')])
    tbl_Notes.add_row([QApplication.translate('HelpDlg','Anything between back ticks ` will show only within 90 seconds before DTs. Example: `~E1 `DTs~dDTs sec`')])
