...
def content() -> str:
    ...
    tbl_EventIndex.add_row(['CHARGE, DRY, DTs, FCe, SCs, SCe, DROP',QApplication.translate('HelpDlg','Index of the corresponding event of the profile to retrieve time and values from the corresponding data structures. Evaluates to -1 if not set.')])
    tbl_EventIndex.add_row(['bCHARGE, bDRY, bDTs, bFCe, bSCs, bSCe, bDROP',QApplication.translate('HelpDlg','Index of the corresponding event of the background profile to retrieve time and values from the corresponding data structures. Evaluates to -1 if not set.')])
    tbl_EventIndex.add_row(['&#160;','&#160;'])
    tbl_EventIndex.add_row(['dCHARGE, dDRY, dDTs, dFCe, dSCs, dSCe, dDROP',QApplication.translate('HelpDlg','Time distance in seconds after the corresponding event. Thus dCHARGE is bound to the current roast time (after CHARGE) in seconds while t is bound to the time in seconds from the start of the recording.')])
    ...
    tbl_Predictions.add_row(['pDTs',QApplication.translate('HelpDlg','Same as pDRY, just for the DTs event.')])
    ...
    tbl_Predictionsbottom.add_row([QApplication.translate('HelpDlg','Note: The same rules as for the corresponding PhasesLCDs apply to pDRY and pDTs:')+newline+QApplication.translate('HelpDlg','\nIf there is no background profile the DRY or DTs bean temperature used for the prediction is taken from the Config>Phases setup.')+newline+QApplication.translate('HelpDlg','\nIf there is a background profile and there is DRY or DTs event in the background profile, the DRY or DTs bean temperature used for the prediction is taken from the background profile.')+newline+QApplication.translate('HelpDlg','\nException to the above for DRY only: if AutoDRY is checked the DRY temperature used for the prediction is taken from the Config>Phases setup.  This does not apply to DTs and AutoDTs.')+newline+QApplication.translate('HelpDlg','\nThe prediction value is the calculated time in seconds to reach the DRY or DTs temperature.')])
