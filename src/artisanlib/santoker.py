...
class Santoker(AsyncComm):
    ...
    __slots__ = [ 'HEADER', '_charge_handler', '_dry_handler', '_DTs_handler', '_scs_handler', '_drop_handler', '_board', '_bt', '_et',
                    '_bt_ror', '_et_ror', '_ir',
                    '_power', '_air', '_drum', '_CHARGE', '_DRY', '_DTs', '_SCs', '_DROP', '_connect_using_ble', '_ble_client' ]
    ...
    def __init__(self, host:str = '127.0.0.1', port:int = 8080, serial:Optional['SerialSettings'] = None,
                ...
                DTs_handler:Optional[Callable[[], None]] = None,
        ...
        self._DTs_handler:Optional[Callable[[], None]] = DTs_handler
        ...
        #
            ...
            if b and b != self._DRY and self._dry_handler is not None:
                try:
                    ...
                    _log.exception(e)
            self._DRY = b
        elif target == self.DTs:
            b = bool(value)
            if b and b != self._DTs and self._DTs_handler is not None:
                try:
                    self._DTs_handler()
                except Exception as e: # pylint: disable=broad-except
                    _log.exception(e)
