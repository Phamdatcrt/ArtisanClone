...
class profileTransformatorDlg(ArtisanDialog):
    def __init__(self, parent:'QWidget', aw:'ApplicationWindow') -> None:
        ...
        # profileTimes: list of DRY, DTs, SCs and DROP times in seconds if event is set, otherwise None
        self.profileTimes:List[Optional[float]] = self.getProfileTimes()
        # list of DRY, DTs, SCs, and DROP target times in seconds as specified by the user, or None if not set
        ...
        # list of CHARGE, DRY, DTs, SCs and DROP BT temperatures
        self.profileTemps = self.getProfileTemps()
        # list of DRY, DTs, SCs, and DROP target temperatures as specified by the user, or None if not set
    ...
    # returns list of DRY, DTs, SCs and DROP profile times in seconds if event is set, otherwise None
    ...
    # returns list of CHARGE, DRY, DTs, SCs and DROP BT temperatures if event is set, otherwise None
    ...
    # returns list of DRY, DTs, SCs and DROP target times in seconds if event is set, otherwise None
    ...
    def phasesTableColumnHeaderClicked(self, i:int) -> None:
        if (self.phases_target_widgets_time is not None and
            ...
            if (phases_target_widgets_time_i is not None and phases_target_widgets_time_i.text() != '') or (phases_target_widgets_percent_i is not None and phases_target_widgets_percent_i.text() != ''):
                ...
                back_DTs = self.aw.qmc.timeB[self.aw.qmc.timeindexB[2]]
                ...
                if i == 0:
                    ...
                    # MAILARD
                    s = stringfromseconds(back_DTs - back_dry)
                elif i == 2:
                    s = stringfromseconds(back_drop - back_DTs)
...
#                    pass
    ...
    def getTargetPhasesTimes(self) -> List[Optional[float]]:
        ...
        DTs:float = self.aw.qmc.timex[self.aw.qmc.timeindex[2]] - offset
        ...
        if drop_phases_target_widget_time is not None and drop_phases_target_widget_time.text() != '':
            drop = DTs + stringtoseconds(drop_phases_target_widget_time.text())
            drop_set = True
        elif drop_phases_target_widget_percent is not None and drop_phases_target_widget_percent.text() != '':
            drop = DTs + (float(drop_phases_target_widget_percent.text()) * drop / 100)
            drop_set = True
        ...
        # determine the target DTs time (relative to the target drop of above) if any
        DTs_phases_target_widgets_time = self.phases_target_widgets_time[1]
        DTs_phases_target_widgets_percent = self.phases_target_widgets_percent[1]
        if DTs_phases_target_widgets_time is not None and DTs_phases_target_widgets_time.text() != '':
            DTs = dry + stringtoseconds(DTs_phases_target_widgets_time.text())
            DTs_set = True
        elif DTs_phases_target_widgets_percent is not None and DTs_phases_target_widgets_percent.text() != '':
            DTs = dry + (float(DTs_phases_target_widgets_percent.text()) * drop / 100)
            DTs_set = True

#        return [(dry if dry_set else None),(DTs if DTs_set else None), None, (drop if drop_set else None)]
        # set all unset target times to the profile times
        return [
            (dry if dry_set else (self.aw.qmc.timex[self.aw.qmc.timeindex[1]] - offset)),
            (DTs if DTs_set else (self.aw.qmc.timex[self.aw.qmc.timeindex[2]] - offset)),
            None,
...
#                pass
    ...
    def createPhasesTable(self) -> None:
        ...
        #
        # the phases transformation are only enabled if at least DRY, DTs and DROP events are set
