...
class RoastProfile:
    ...
    def __init__(self, aw:'ApplicationWindow', profile:'ProfileData', filepath:str, color: Tuple[float, float, float, float]) -> None:
        ...
        self.events1:Optional[Sequence[Optional[float]]] = None # ET temperatures of main events [CHARGE, DRY, DTs, FCe, SCs, SCe, DROP], None if not set
        self.events2:Optional[Sequence[Optional[float]]] = None # BT temperatures of main events [CHARGE, DRY, DTs, FCe, SCs, SCe, DROP], None if not set
        self.events_timex:Optional[Sequence[Optional[float]]] = None # roast times of main events [CHARGE, DRY, DTs, FCe, SCs, SCe, DROP] in seconds, None if not set
...
class roastCompareDlg(ArtisanDialog):
    ...
    def __init__(self, parent:QWidget, aw:'ApplicationWindow', foreground:Optional[str] = None, background:Optional[str] = None) -> None:
        ...
        self.alignnames = [
            ...
            QApplication.translate('Label','DTs'),
...
#        selected = self.profileTable.getselectedRowsFast() # does return [1],[2],[1],.. on repeated clicks on the row header of the first entry insteadd of [1],[0],[1],..
    ...
    #   0:CHARGE, 1:TP, 2:DRY, 3:DTs, 4:FCe, 5:SCs, 6:SCe, 7:DROP
    ...
    def getPhasesData(self) -> List[Tuple[str, float, Tuple[float,float,float], bool, bool, str]]:
        ...
        for p in reversed(profiles):
            if p.visible:
                ...
                DTs:float = p.timex[p.timeindex[2]] - start if p.timeindex[2] != 0 else 0
                p1:float = dry
                p3:float = total - DTs if DTs != 0 else 0
