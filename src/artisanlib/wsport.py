...
class wsport:
    ...
    __slots__ = [ 'aw', '_loop', '_thread', '_write_queue', 'default_host', 'host', 'port', 'path', 'machineID', 'lastReadResult', 'channels', 'readings', 'tx',
                    ...
                    'DRY_node', 'DTs_node', 'FCe_node', 'SCs_node', 'SCe_node', 'STARTonCHARGE', 'OFFonDROP', 'open_event', 'pending_events',
                    'ws', 'wst' ]
    ...
    def __init__(self, aw:'ApplicationWindow') -> None:
        ...
        self.DTs_node:str = 'firstCrackBeginningEvent'
            ...
            if self.charge_message != '' and pushMessage == self.charge_message:
                ...
                if self.aw.qmc.flagstart and self.data_node in j:
                    ...
                    if self.event_node in data:
                        ...
                        elif self.aw.qmc.timeindex[2] == 0 and data[self.event_node] == self.DTs_node:
                            # addEvent(DTs) received
                            if self.aw.seriallogflag:
                                self.aw.addserial('wsport message: addEvent(DTs) processed')
                            self.aw.qmc.markDTsSignal.emit(False)
