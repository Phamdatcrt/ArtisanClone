...
#                            heater_event = True
        ...
        if last_item is not None and timeindex[0]>-1 and 'adfc_timestamp' in last_item:
            ...
            # DTs_time is the time of DTs in seconds after CHARGE
            DTs_time = float(last_item['adfc_timestamp'])
            if DTs_time > 0:
                # if DTs_time is given
                DTs_idx = int(round(DTs_time + timeindex[0]))
                if 0 < DTs_idx < len(timex):
                    timeindex[2] = DTs_idx
