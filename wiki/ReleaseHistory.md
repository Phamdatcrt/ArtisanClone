...
* ADDITIONS
  ...
  - adds fields `~dryphasedeltatemp`, `~midphasedeltatemp`, `~finishphasedeltatemp`, `~DTstime`, and `~DTstime_long` to autosave
...
* CHANGES
  ...
  - PhasesLCDs show time to DTs if DRY is not set after DRY target passed
* DELETIONS
...
* NEW FEATURES
  ...
  - adds Artisan Commands: `popup(m[,t])`, `message(s)`, `setCanvasColor(c)`, `resetCanvasColor`, `button(e)` with `e = { START, CHARGE, DRY, DTs, FCe, SCs, SCe, DROP, COOL, OFF }`, `p-i-d(p,i,d)`, `pidSV(v)`, `pidRS(rs)`, `pidSource(n)`, `palette(p)`, `loadBackground(<filepath>)`, `clearBackground`, `alarmset(as)`, `adjustSV(n)`
...
* CHANGES
  ...
  - adds DTs ROR to computed section of `.alog` Artisan Profiles
...
* FIXES
  ...
  - UI: deactivate autoDRY and autoDTs for the current recording after the added event was undone to avoid further auto event triggers
...
* NEW FEATURES
  ...
  - adds flag to allow phases to be adjusted based on DRY and DTs of the background profile
...
* NEW FEATURES
 ...
 Note that without optimal smoothing active, temperature curves are not smoothed at all.
  ...
  - adds crash reporter
  - adds expected time to DRY and DTs to phases LCDs in percentage  and temperature mode
* CHANGES
...
* NEW FEATURES
  ...
  * adds "Annotations" flag to hide annotations on BT (CHARGE, DTs, ..) (event dialog)
...
* NEW FEATURES
  ...
  * adds flag to align of background profiles also w.r.t. DTs
...
* FIXES
  ...
  * fixes early autoDRY and autoDTs
...
* NEW FEATURES
  ...
  * adds autoDRY and autoDTs to phases dialog
  * adds phase LCDs (TP, DRY, DTs) estimating time to the event and counting time after the event
  ...
  * alarms
    ...
    * adds new alarm actions to trigger default event actions like START, DTs,..
...
* Events
  ...
  * allows events like DTs, FCe,.. (but for CHARGE) to be removed by inserting 0:00 as event time
* Alarms
  ...
  * adds time-based alarms triggered at the given seconds after the specified event like TP, DTs,..
...
* added sync of mid phase with the DRY END and DTs events
